...
import { SOCIAL_MEDIA, PHONE_NUMBER } from '@/constants/contact'
...
const ContactSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const messageType = sanitizedData.type === 'mayorista' ? 'compra mayorista / reventa' : 'compra minorista'
      const whatsappMessage = `¡Hola! Me contacto desde el formulario de Bonnilda 🍑

📝 *Mis datos:*
• Nombre: ${sanitizedData.name}
• WhatsApp: ${sanitizedData.whatsapp}
• Tipo de compra: ${messageType}

${sanitizedData.message ? `💬 *Mensaje:*\n${sanitizedData.message}` : ''}

¡Espero su respuesta! Gracias 😊`

      // Abrir WhatsApp con el mensaje pre-cargado
      const whatsappURL = SOCIAL_MEDIA.whatsapp.urlWithMessage(whatsappMessage)
      ...
      analytics.trackFormSubmit(sanitizedData.type as 'minorista' | 'mayorista')
      ...
      setSubmitMessage('¡Perfecto! Se abrió WhatsApp con tu mensaje. Solo tenés que presionar enviar 🚀')
            ...
            <Card padding="lg">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <p className="text-sm text-text-muted text-center">
                  ...
                  <a 
                    href={SOCIAL_MEDIA.whatsapp.urlWithMessage('Hola! Me interesa conocer más sobre los bombones Bonnilda 🍑')}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => analytics.trackWhatsAppClick('contact')}
                    className="text-accent-peach hover:text-accent-raspberry font-medium transition-colors"
                  >
